...
# Usage: bash .trae/skills/project-page-builder/scripts/check.sh <api-path> <views-path>
...
if [ -n "$RELATIVE" ]; then
  echo "⚠️  Found relative paths (sub-module imports like '../data' are acceptable):"
  echo "$RELATIVE"
else
...
# ── 5. Check i18n ─────────────────────────────────────
echo ""
echo "📌 Step 5: Check for i18n usage"
HARDCODED_CN=$(grep -rn "[\u4e00-\u9fff]" "$VIEW_PATH" "$API_PATH" --include="*.vue" --include="*.ts" 2>/dev/null | grep -v "//.*[\u4e00-\u9fff]" | grep -v "locales" | head -20)
if [ -n "$HARDCODED_CN" ]; then
  echo "⚠️  Found hardcoded Chinese text, recommend using i18n $t():"
  echo "$HARDCODED_CN"
else
